...
class Processor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        """
        good = n_photons >= 2
        # Trigger (pair-matched photons)
        pair_photon1_pt_rel = pair_photons.pt[:, 0] / pair_diphoton_mass
        pair_photon2_pt_rel = pair_photons.pt[:, 1] / pair_diphoton_mass
        trigger = ((pair_photons.pt[:, 0] > 35) & (pair_photons.pt[:, 1] > 25)) | (pair_photons.pt[:, 0] > 140)  # fmt: skip
        good = good & trigger
        # Rel pT cut (pair diphoton)
        rel_pt_cut = (pair_photon1_pt_rel > 0.4) & (pair_photon2_pt_rel > 0.3)
        good = good & rel_pt_cut
        # Myy mass window (pair diphoton)
        myy_cut = (pair_diphoton_mass > 105) & (pair_diphoton_mass < 160)
        good = good & myy_cut
